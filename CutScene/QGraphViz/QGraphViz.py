...
"""
from PySide2.QtWidgets import QApplication, QWidget, QScrollArea, QSizePolicy
from PySide2.QtGui import QPainter, QPen, QBrush, QColor, QPainterPath, QImage, QLinearGradient
        ...
        if(node in graph.nodes):
            ...
            for edge in node.in_edges:
                ...
                if edge.source.parent_graph == edge.dest.parent_graph:
                    try:
                        if(edge in edge.source.parent_graph.edges):
                ...
                if(self.hovered_Node is None):
                    ...
                    if(self.hovered_Node is not None):
                        if "width" in list(self.hovered_Node.kwargs.keys()):
                            self.hovered_Node_Back_width=int(self.hovered_Node.kwargs["width"])
                        else:
                ...
                if(self.hovered_Edge is None):
                    ...
                    if(self.hovered_Edge is not None):
                        if "width" in list(self.hovered_Edge.kwargs.keys()):
                            self.hovered_Edge_Back_width=int(self.hovered_Edge.kwargs["width"])
                        else:
    ...
    def paintGraph(self, graph, painter):
        ...
        for node in graph.nodes:
            if type(node)==Node:
                ...
                painter.setPen(pen)
                painter.setBrush(brush)
                if("label" in node.kwargs.keys() and "hidelabel" not in node.kwargs.keys()):
                    ...
                    for t in txt:
                        ...
                        txt_width=rect.width() if rect.width()>txt_width else txt_width
                        txt_height+=rect.height()
                    txt_width+=self.engine.margins[0]
                    txt_height+self.engine.margins[1]
                else:
                    txt_width=node.size[0]
                    txt_height=node.size[1]
                ...
                if("shape" in node.kwargs.keys()):
                    if(node.kwargs["shape"]=="box"):
                        ...
                        node.size[0] = node.size[1] if node.size[1]> node.size[0] else node.size[0]
                        node.size[1] = node.size[1] if node.size[1]> node.size[0] else node.size[0]
                        ...
                        width=txt_width+txt_width*0.5
                        height=txt_height+txt_height*0.1
                    ...
                    width=txt_width+txt_width*0.2
                    height=txt_height+txt_width*0.2
